...
<script setup lang="ts">
  ...
  const computeSelectedNames = () => {
    // we want to select names with resp. attr.
    const filteredNames = names
      //filter for selected gender
      .filter((name) => name.gender === options.gender)
      //filter for selected popularity
      .filter((name) => name.popularity === options.popularity)
      .filter((name) => {
        //if any length return all
        if (options.length === Length.ALL) return true
        else {
          //else return resp. length of the options
  ...
  console.log(selectedNames, 'OUR Options')
</script>

<template>
  <div class="container">
    <h1>Baby Name Generator</h1>
    <p>Choose your option and click the "Find Name" button below.</p>
